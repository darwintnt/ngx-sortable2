...
function isFormArray<T>(
  target: SortableData<T>
): target is SortableData<T> & FormArray<any> {
  ...
  return (
    !!target.at &&
    !!(target as FormArray).insert &&
    !!(target as FormArray).reset
  );
}

export class SortablejsBinding<T> {
  constructor(private readonly target: SortableData<T>) {}
  ...
  get(index: number) {
    return isFormArray(this.target)
      ? (this.target.at(index) as T)
      : (this.target as T[])[index];
  }
